...
class _OnboardingPageState extends ConsumerState<OnboardingPage> {
  ...
  static const _stacks = [
    _StackOption(
      id: 'onepage',
      label: 'One Page',
      desc: 'Une page défilante avec sections. Idéal pour landing, portfolio, lancement.',
    ...
    _StackOption(
      id: 'multipage',
      label: 'Multi Page',
      desc: 'Pages reliées avec navigation. Idéal pour entreprises, e-commerce, ASBL.',
  ...
  static const _sectors = [
    ('Personnalisé', Icons.tune),
    ('Restaurant / Café / Bar', Icons.restaurant),
    ('Cabinet médical / Kiné', Icons.local_hospital),
    ('E-commerce / Boutique', Icons.shopping_bag),
    ('Portfolio / Agence créative', Icons.brush),
    ('Fitness / Sport / Bien-être', Icons.fitness_center),
    ('Juridique / Notaire', Icons.gavel),
    ...
    ('Artisan / BTP', Icons.construction),
    ('Tech / SaaS / Startup', Icons.code),
  ...
  List<_SectionConfig> _buildSectionCatalogue(String sector) {
    final all = [
      ...
      _SectionConfig(type: 'services', label: 'Services / Offres', icon: Icons.grid_view, color: const Color(0xFF8B5CF6), enabled: true, data: {'title': 'Nos services', 'items': <Map<String, String>>[]}),
      ...
      _SectionConfig(type: 'pricing', label: 'Tarifs / Plans', icon: Icons.sell_outlined, color: const Color(0xFFEC4899), enabled: false, data: {'title': 'Nos tarifs', 'plans': <Map<String, dynamic>>[]}),
      _SectionConfig(type: 'faq', label: 'FAQ', icon: Icons.help_outline, color: const Color(0xFF6366F1), enabled: false, data: {'items': <Map<String, String>>[]}),
      ...
      _SectionConfig(type: 'team', label: 'Équipe / Membres', icon: Icons.people_outline, color: const Color(0xFFA855F7), enabled: false, data: {'title': 'Notre équipe', 'members': <Map<String, String>>[]}),
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Erreur logo: $e')),
        );
      }
    }
  }
    ...
    return _GlassPanel(
      ...
      child: Column(
        children: [
          ...
          Text('Crée ton site avec SedApps',
      ...
      child: Column(
        ...
        children: [
          _Header(
            ...
            subtitle: _premium ? 'Premium 10K — $_projectType' : '${_stack == 'onepage' ? 'One Page' : 'Multi Page'} — $_projectType',
            color: _premium ? const Color(0xFFF59E0B) : (_stack == 'onepage' ? const Color(0xFF3B82F6) : const Color(0xFF8B5CF6)),
            scale: scale,
          ),
          ...
          TextField(
            ...
            style: TextStyle(color: Colors.white, fontSize: 16 * scale),
            decoration: _darkInput('Domaine personnalisé (optionnel)', 'monsite.fr', Icons.language),
          ),
          SizedBox(height: 6 * scale),
          Text('Domaine auto : $_domain',
  ...
  Widget _stepBrandIdentity(double scale) {
    ...
    return _GlassPanel(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          _Header(icon: Icons.palette_outlined, title: 'Identité de marque', subtitle: 'Couleurs, typo, ton et logo', color: const Color(0xFF8B5CF6), scale: scale),
          SizedBox(height: 24 * scale),
            ...
            children: _tones.map((t) {
              ...
              return GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    color: sel ? const Color(0xFF8B5CF6).withValues(alpha: 0.24) : Colors.white.withValues(alpha: 0.06),
                    borderRadius: BorderRadius.circular(999),
                    border: Border.all(color: sel ? const Color(0xFF8B5CF6) : Colors.white.withValues(alpha: 0.12), width: sel ? 1.5 : 1),
                  ),
                  child: Text(t.$2, style: TextStyle(
                    color: sel ? const Color(0xFFc4b5fd) : Colors.white.withValues(alpha: 0.70),
