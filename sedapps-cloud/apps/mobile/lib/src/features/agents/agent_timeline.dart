          ...
          children: [
            Row(
              children: [
                ...
                Expanded(child: Text('Interaction des agents IA', style: Theme.of(context).textTheme.titleLarge)),
                Chip(label: Text(job!.status)),
              ],
            ),
            const SizedBox(height: 12),
            if (agents.isEmpty) const Text('Aucun agent exécuté pour le moment.'),
            for (var i = 0; i < agents.length; i++) _AgentTile(index: i + 1, agent: agents[i]),
          ],
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: ExpansionTile(
        ...
        subtitle: Text('${agent.model} · ${agent.durationMs} ms · ${agent.tokensIn + agent.tokensOut} tokens'),
        trailing: Chip(label: Text(agent.status)),
        childrenPadding: const EdgeInsets.fromLTRB(16, 0, 16, 16),
        children: [
          if (agent.warnings.isNotEmpty)
            Align(alignment: Alignment.centerLeft, child: Text('Warnings: ${agent.warnings.join(', ')}')),
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Interaction des agents IA', style: Theme.of(context).textTheme.titleLarge),
            const SizedBox(height: 8),
            const Text('Lance une génération pour voir chaque agent travailler : designer, copywriter, SEO, frontend, CMS, formulaire, analytics et QA.'),
          ],
        ),
      ),
    );
