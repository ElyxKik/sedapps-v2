...
class _NewSitePageState extends ConsumerState<NewSitePage> {
  ...
  Future<void> _generate() async {
    if (_business.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Le nom du business est requis')));
      return;
    }
    setState(() => _loading = true);
    try {
      final api = ref.read(apiClientProvider);
      final project = await api.createProject(_business.text.trim(), _sector.text.trim().isEmpty ? null : _sector.text.trim());
      ...
      children: [
        LayoutBuilder(
          builder: (context, constraints) {
            ...
            if (!wide) {
              return Column(children: [form, const SizedBox(height: 20), agents]);
            }
            return Row(
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Décris ton projet', style: Theme.of(context).textTheme.titleLarge),
            const SizedBox(height: 4),
            const Text('Plus tu es détaillé, meilleur sera le résultat.',
                style: TextStyle(color: AppColors.textSecondary)),
            const SizedBox(height: 24),
            Text('Nom du business *', style: Theme.of(context).textTheme.titleSmall),
            const SizedBox(height: 6),
            TextField(
              controller: business,
              decoration: const InputDecoration(
                hintText: 'Ex: Mon Agence Web',
                prefixIcon: Icon(Icons.business),
              ),
            ),
            const SizedBox(height: 18),
            Text('Secteur d\'activité', style: Theme.of(context).textTheme.titleSmall),
            const SizedBox(height: 6),
            TextField(
              controller: sector,
              decoration: const InputDecoration(
                hintText: 'Ex: Services numériques, Restauration...',
                prefixIcon: Icon(Icons.category_outlined),
              ),
            ),
            const SizedBox(height: 18),
            Text('Brief détaillé', style: Theme.of(context).textTheme.titleSmall),
            const SizedBox(height: 6),
            TextField(
              controller: brief,
              maxLines: 6,
              decoration: const InputDecoration(
                hintText: 'Décris ce que tu vends, à qui, avec quel style, tes objectifs...',
            ...
            if (loading)
              Column(
                children: const [
                  ...
                  SizedBox(height: 12),
                  Text('Création en cours... Nos agents travaillent sur ton site.',
            ...
            FilledButton.icon(
              ...
              label: Text(loading ? 'Création en cours...' : 'Lancer la création IA'),
...
class _AgentsSection extends StatelessWidget {
  ...
  static const _agents = [
    ('Designer', Icons.palette, 'Palette, typo, layout', Color(0xFF6366F1)),
    ('Copywriter', Icons.edit_note, 'Titres, descriptions, CTA', Color(0xFFEC4899)),
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Les agents IA', style: Theme.of(context).textTheme.titleLarge),
            const SizedBox(height: 4),
...
class _AgentCard extends StatelessWidget {
  const _AgentCard({required this.name, required this.icon, required this.desc, required this.color});
  final String name;
  final IconData icon;
  final String desc;
  final Color color;
