...
class _ProjectLoadingPageState extends ConsumerState<ProjectLoadingPage>
  ...
  int _progress = 0;
  String _currentLabel = 'Votre site est en cours de création…';
  bool _navigating = false;

  static const _waitMessages = [
    'Notre IA analyse votre secteur…',
    'Choix de la typographie idéale…',
    'Construction de la structure visuelle…',
    'Rédaction des textes adaptés à votre marque…',
        ...
        if ((status == 'success' || status == 'degraded') && !_navigating) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                  content:
                      Text('Erreur : ${job['error'] ?? 'génération échouée'}')),
            );
          }
        }
      } catch (_) {
  ...
  String _friendlyLabel(String label, String step) {
    ...
    const fallbacks = {
      'flow_select': 'Préparation de votre site…',
      ...
      'blog_writer': 'Rédaction des articles de blog…',
    };
    return fallbacks[step] ?? 'Votre site est en cours de création…';
  }
