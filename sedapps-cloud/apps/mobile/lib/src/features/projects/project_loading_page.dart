...
class _ProjectLoadingPageState extends ConsumerState<ProjectLoadingPage>
  ...
  int _progress = 0;
  String _currentLabel = 'Initialisation…';
    ...
    _pollTimer = Timer.periodic(const Duration(milliseconds: 1200), (_) async {
      try {
        ...
        for (final raw in events) {
          ...
          if (existing == null) {
            _steps[step] = _LiveStep(step: step, label: label, status: status, progress: progress);
            _stepOrder.add(step);
          } else {
        ...
        if (status == 'success' && !_navigating) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('Erreur : ${job['error'] ?? 'génération échouée'}')),
            );
          }
        }
      } catch (_) {
  ...
  Widget _buildHeader() {
    return Column(
      children: [
        ...
        const Text(
          'Génération du site par DeepSeek en cours…',
  ...
  Widget _buildProgressBar() {
    return ClipRRect(
      ...
      child: LinearProgressIndicator(
        ...
        valueColor: AlwaysStoppedAnimation<Color>(const Color(0xFF22D3EE).withOpacity(0.9)),
      ),
    );
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              s.label.isEmpty ? s.step : s.label,
