...
class ProjectWorkspaceNotifier extends StateNotifier<ProjectWorkspaceState> {
          ...
          const ProjectWorkspaceState(
            sections: [
              SiteSection(
                  id: 'hero',
                  title: 'Hero',
                  content: 'Créez votre présence web avec une équipe experte.'),
              SiteSection(
                  id: 'features',
                  title: 'Services',
                  content: 'Design, développement, SEO et maintenance.'),
              SiteSection(
                  id: 'faq',
                  title: 'FAQ',
                  content: 'Réponses aux questions fréquentes de vos clients.'),
              SiteSection(
                  id: 'contact',
                  title: 'Contact',
                  content: 'Formulaire de contact et coordonnées.'),
            ],
            primaryColor: '#0EA5E9',
            headingFont: 'Inter',
            articles: [
              ...
              CmsArticle(
                  ...
                  markdown:
                      '# Responsive design\n\nOptimise ton site pour tous les écrans.',
                  status: 'draft'),
            ],
            ...
            publish: PublishState(
                ...
                domain: 'monsite.sedapps.cloud',
                status: 'draft'),
            messages: [
              ChatMessage(
                role: 'assistant',
                text:
                    'Bienvenue ! Je suis le Générateur IA SedApps. Demande-moi de modifier ton site, créer un article CMS, ajuster les couleurs, préparer un déploiement…',
  ...
  void createArticle() {
    ...
    final article = CmsArticle(
        id: id,
        title: 'Nouvel article',
        markdown: '# Nouvel article\n\nCommence à écrire ici.',
        status: 'draft');
    state = state.copyWith(
  ...
  void updatePublish(
      {bool? subdomainEnabled, bool? customDomainEnabled, String? domain}) {
    state = state.copyWith(
  ...
  void syncFromProject(Map<String, dynamic> project) {
      ...
      for (final item in rawSections) {
        if (item is Map) {
          ...
          sections.add(SiteSection(
              id: id, title: title, content: content, enabled: enabled));
        }
      }
    }
    ...
    if (tokens is Map) {
      ...
      if (tokenPrimary is String && tokenPrimary.startsWith('#'))
    ...
    state = state.copyWith(
      ...
      publish: state.publish
          .copyWith(domain: domain, url: publishedUrl, status: status),
      hasUnsavedChanges: false,
      previewNonce: nonce,
    );
  }
  ...
  Future<void> publishSite() async {
    ...
    state = state.copyWith(
        publish: state.publish.copyWith(
            status: 'published', url: 'https://${state.publish.domain}'));
  }
  ...
  Future<void> askAi(String text) async {
    ...
    if (lower.contains('seo') ||
        lower.contains('meta') ||
        lower.contains('mots clés')) {
      tools.add('seo.optimize');
      agents.add('SEO Specialist');
    }
    if (lower.contains('publie') ||
        lower.contains('déploie') ||
        lower.contains('deploy') ||
        lower.contains('mise en ligne')) {
    ...
    if (tools.contains('design.update')) {
      if (lower.contains('rouge'))
        updateDesign(primaryColor: '#EF4444');
      else if (lower.contains('vert'))
        updateDesign(primaryColor: '#10B981');
      else if (lower.contains('violet'))
        updateDesign(primaryColor: '#7C3AED');
      else if (lower.contains('orange'))
        updateDesign(primaryColor: '#F59E0B');
      else if (lower.contains('bleu') || lower.contains('couleur'))
        updateDesign(primaryColor: '#2563EB');
    }
    ...
    if (idx >= 0) {
      updated[idx] = pending.copyWith(
          text: responseText, isPending: false, isError: isError);
    } else {
