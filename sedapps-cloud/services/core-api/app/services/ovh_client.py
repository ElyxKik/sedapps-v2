...
class OvhClient:
    ...
    def __init__(self) -> None:
        self.enabled = bool(
            settings.OVH_APP_KEY
            and settings.OVH_APP_SECRET
            and settings.OVH_CONSUMER_KEY
        )
        self._client = None
        if self.enabled:
            try:
    ...
    def is_domain_available(self, domain: str) -> bool:
        ...
        if not self.enabled or not self._client:
            logger.warning("OVH Client credentials not configured or initialization failed. Defaulting domain availability to True.")
            return True
        ...
        try:
            ...
            results = self._client.get(f"/domain/register/search", domain=domain)
            for res in results:
