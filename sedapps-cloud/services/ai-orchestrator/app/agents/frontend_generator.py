...
ALLOWED_SECTIONS = {
    ...
    "testimonials.carousel", "pricing.table", "faq.accordion",
    "cta.banner", "form.contact", "blog.index", "richtext", "gallery.grid",
}
    ...
    def _build_schema(self, inp: AgentInput) -> dict[str, Any]:
        ...
        for p in copy.get("pages", []):
            ...
            for s in p.get("sections", []):
                rtype = COPY_TO_RENDERER.get(s.get("type"))
                if not rtype or rtype not in ALLOWED_SECTIONS:
                    continue
