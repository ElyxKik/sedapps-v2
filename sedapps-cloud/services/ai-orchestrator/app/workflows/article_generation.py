...
async def _orchestrate(job_id: str, project_id: str, tenant_id: str,
    ...
    core.record_run(job_id, {
        ...
        "input": params, "output": out.data, "warnings": out.warnings,
    })
    core.job_complete(job_id, {
        "status": "success" if out.status == "ok" else "degraded",
        "error": None, "output": {"article": out.data},
        "tokens_in": out.tokens.prompt, "tokens_out": out.tokens.completion,
    })
    return out.data
...
def run_article_generation(self, job_id: str, project_id: str, tenant_id: str,
                           params: dict[str, Any]) -> dict[str, Any]:
