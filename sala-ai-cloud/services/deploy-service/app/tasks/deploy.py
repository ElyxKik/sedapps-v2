...
from app.services.build import build_static_site, cleanup_workdir, prepare_static_files_workspace, prepare_workspace, validate_deployment_dir
...
def deploy_site(self, body: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        if payload.get("render_mode") == "static_classic":
            log.info("deployment %s prepare static files workspace", deployment_id)
            out_dir = prepare_static_files_workspace(deployment_id, payload.get("files", []))
        else:
        ...
        validation_issues = validate_deployment_dir(out_dir)
        if validation_issues:
            log.warning("deployment %s validation issues: %s", deployment_id, validation_issues)
            core.deployment_update(deployment_id, {"status": "validation_failed", "domain": domain, "validation_issues": validation_issues})
        ...
        if not custom_domain:
            ...
            core.deployment_update(deployment_id, {"status": "dns_configuring", "domain": domain})
