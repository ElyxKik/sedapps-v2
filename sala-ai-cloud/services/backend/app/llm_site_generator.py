...
def fallback_site_payload(brief: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any], list[dict[str, Any]], list[dict[str, str]]]:
    ...
    files = _enforce_stack_files(files, brief, design_tokens)
    output = {
...
<body>
  <header><h1>{business}</h1><p>{sector}</p><nav><a href=\"index.html\">Accueil</a> <a href=\"services.html\">Services</a> <a href=\"about.html\">À propos</a> <a href=\"contact.html\">Contact</a></nav></header>
  <main>
    <section><h2>{title}</h2><p>{description}</p></section>
    <section><h2>Pourquoi nous choisir</h2><p>Une approche professionnelle, structurée et orientée résultats.</p></section>
  </main>
  <script src=\"./script.js\"></script>
</body>
</html>"""
...
def _enforce_stack_files(
    ...
    def _is_support_file(path: str) -> bool:
        """Files that belong to the modular component architecture and must never be dropped."""
        p = path.lower()
        return (
            p == "app.js"
            or p.startswith("components/")
            or p.startswith("data/")
            or p.startswith("assets/")
            or p.endswith(".css")
            or p.endswith(".js")
            or p.endswith(".json")
            or p.endswith((".svg", ".png", ".jpg", ".jpeg", ".webp", ".ico", ".woff", ".woff2"))
        )

    if stack != "multipage":
        # One-page site: keep index.html + every modular support file, drop only EXTRA top-level
        # HTML pages (about.html, services.html, ...) which don't belong to a single-page layout.
        kept: list[dict[str, str]] = []
        for f in files:
            path = f.get("path") or ""
            if path == "index.html" or _is_support_file(path):
                kept.append(f)
            # extra *.html pages are intentionally skipped in one-page mode
        return kept

    business = brief.get("business_name") or brief.get("name") or "Mon Site"
    sector = brief.get("sector") or "Services"
    description = brief.get("brief") or brief.get("description") or f"Site moderne pour {business}"
    by_path = {f.get("path"): f for f in files if isinstance(f, dict) and f.get("path")}
    for path, title in {
        ...
        if path not in by_path:
            by_path[path] = {
                "path": path,
                "content": _fallback_page_html(business, sector, title, description, tokens),
            }
    ordered = ["index.html", "services.html", "about.html", "contact.html", "styles.css", "script.js"]
    return [by_path[p] for p in ordered if p in by_path] + [
        f for p, f in by_path.items() if p not in set(ordered)
    ]
...
def _deepseek_call(system_prompt: str, user_payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, int], int]:
    ...
    fallback = "deepseek-v4-flash" if primary != "deepseek-v4-flash" else "deepseek-chat"
    primary_timeout = 300 if primary == "deepseek-v4-pro" else 120
    max_retries = 3
    last_exc = None
    
    # Tentative sur le modèle principal avec retries automatiques (gestion du backoff)
    for attempt in range(max_retries):
        try:
            return _deepseek_try(primary, system_prompt, user_payload, timeout=primary_timeout)
        except Exception as exc:
            last_exc = exc
            # Si c'est une réponse vide ou une erreur HTTP temporaire, on réessaie
            if "empty" in str(exc).lower() or "vide" in str(exc).lower() or isinstance(exc, (httpx.HTTPError, RuntimeError)):
                time.sleep(1.5 * (attempt + 1))  # Backoff progressif
                continue
            raise
            
    # Auto-fallback rapide si le modèle principal échoue après ses retries
    if fallback:
...
_DESIGN_SYSTEM_PROMPT = """You are a senior UI Designer specialized in premium modern web design systems.
...
Quality bar:
- Every generated website must look professional, premium and agency-grade
- Aim for a polished SaaS/startup aesthetic inspired by Linear, Stripe, Notion and high-end product websites
- Avoid cheap template aesthetics, generic gradients, weak spacing, low contrast or inconsistent components
- The design must feel custom-made for the user's business sector, not generic
- Use strong visual hierarchy, refined typography, generous whitespace, elegant cards, subtle depth and premium micro-interactions
...
- Never produce a generic/basic/low-end visual system
- Use premium typography pairings and explicit spacing scale
- Define components that feel polished: buttons, cards, inputs, navigation and footer
...
_FRONTEND_BUILDER_PROMPT = """You are an expert senior Frontend Engineer specialized in modern zero-build static websites built with a MODULAR, COMPONENT-BASED JavaScript architecture.

You must generate production-ready code using ONLY:
- HTML5
- CSS3
- Vanilla JavaScript with NATIVE ES MODULES (import/export)
- Native Web Components if useful
- Alpine.js via CDN only if explicitly useful for interactions
- HTMX via CDN only if explicitly useful for dynamic content loading

Forbidden:
- Backend frameworks
- Server-side code
- Node.js dependencies, npm packages, package.json
- Build tools or compilation steps (no Vite, Next.js, Nuxt, React, Vue, Angular, Svelte, Astro)
- JSX/TSX/TypeScript
- Importing dependencies from node_modules

The result must be directly deployable on static hosting such as Cloudflare Pages, Netlify, GitHub Pages or any basic static file server, with NO build step.
...
You MUST produce a component-based architecture. Each UI block is an independent, reusable, data-driven JS module that returns clean HTML. Required project structure:

project/
├── index.html
├── app.js                  (entry: imports components + site data, mounts the page)
├── data/
│   └── site-data.js        (exports a single `siteData` object: all content, no hard-coded text in components)
├── components/
│   ├── header.js
│   ├── hero.js
│   ├── features.js         (or services.js, depending on sector)
│   ├── testimonials.js
│   ├── contact.js
│   ├── footer.js
│   └── ...                  (add pricing.js, faq.js, cta.js, gallery.js, stats.js, etc. when relevant)
└── assets/
    └── css/
        └── styles.css
...
5. at least: components/header.js, components/hero.js, components/footer.js (+ all sections in the UX architecture)

### index.html rules
- Clean semantic HTML5, lang attribute matching the copy language
- In <head>: link the stylesheet `<link rel="stylesheet" href="./assets/css/styles.css">`, meta description, viewport, og:title, og:description, og:image and JSON-LD Organization
- A single mount point in <body>, e.g. `<div id="app"></div>`
- Load the entry script as a MODULE before </body>: `<script type="module" src="./app.js"></script>`
- Do NOT inline section HTML in index.html — sections are rendered by components via app.js

### app.js rules (entry/assembler)
- `import { siteData } from './data/site-data.js';`
- `import { Header } from './components/header.js';` etc. (relative paths with `.js` extension, required for native ES modules)
- Assemble the page by concatenating component output, e.g.:
  const app = [Header(siteData.header), Hero(siteData.hero), Features(siteData.features), Footer(siteData.footer)].join('');
  document.getElementById('app').innerHTML = app;
- After mount, wire global interactions (smooth scroll, dark mode toggle with localStorage, mobile menu, IntersectionObserver scroll reveal, contact form mailto fallback). Always check element existence before binding.

### components/*.js rules
- Each file exports ONE named function matching the component (e.g. `export function Hero(data) { ... }`)
- The function RECEIVES its data as a parameter and returns an HTML string — NO hard-coded content, NO direct DOM queries inside the template
- Add a short JSDoc comment documenting the expected `data` shape
- Components must be independent, reusable, configurable and decoupled from content
- Section components must set the correct `id` so nav anchors resolve (ids must match the UX architecture)

### data/site-data.js rules
- `export const siteData = { ... };` containing every piece of content keyed per component (header, hero, features, testimonials, pricing, faq, cta, contact, footer, seo, meta)
- This is the SINGLE source of content. Components read only from the data passed in.

### Quality bar
- Mobile-first responsive (media queries 768px and 1024px), premium SaaS/startup aesthetic inspired by Linear, Stripe, Notion, Vercel, Framer
- Strong visual hierarchy, generous spacing, refined cards, subtle depth, premium micro-interactions and scroll animations
- Avoid default browser-looking components, unstyled forms, flat generic cards, cramped spacing
- Each project must have its OWN identity — never a generic Bootstrap-like template
- Dark mode toggle persisted via localStorage
- Contact form works without backend (mailto fallback) unless client-only Supabase/Firebase is explicitly requested
- Accessibility: ARIA, semantic tags, alt text, focus-visible
- No external deps except Google Fonts, Alpine.js CDN, HTMX CDN, Supabase/Firebase client CDN when explicitly useful
- No Lorem Ipsum, no TODO, no placeholder content
- styles.css uses CSS variables from the design system (--primary, --secondary, --accent, --bg, --surface, --text, --text-muted, --radius, --shadow ...)

IMPORTANT:
- Zero-build: serving the folder statically must work. Native ES modules require correct relative paths WITH the `.js` extension.
- Do not include markdown fences. Do not include deployment instructions inside files.

Output format (STRICT JSON, no markdown). Use the `files` array with explicit paths:

{
  "files": [
    {"path": "index.html", "content": "<!doctype html>..."},
    {"path": "app.js", "content": "import { siteData } from './data/site-data.js'; ..."},
    {"path": "data/site-data.js", "content": "export const siteData = { ... };"},
    {"path": "components/header.js", "content": "export function Header(data) { return `...`; }"},
    {"path": "components/hero.js", "content": "export function Hero(data) { return `...`; }"},
    {"path": "components/footer.js", "content": "export function Footer(data) { return `...`; }"},
    {"path": "assets/css/styles.css", "content": ":root {...} ..."}
  ]
}"""
...
You must:
- Verify the website is 100% static and zero-build
- Detect forbidden frameworks or build dependencies
- Detect UX issues
...
- Treat generic/basic/cheap-looking design as a major UI issue
- Treat weak visual hierarchy, cramped spacing, inconsistent radii/shadows or unstyled controls as UI issues
- Treat a site that does not feel premium/professional as overall_score <= 70
- Any React, Next.js, Vue, Angular, Svelte, Astro, Vite, Node.js, npm, package.json or build-step dependency is a critical issue
- Any missing dark mode toggle is a UX issue
- Any missing reusable header/footer pattern is a UI architecture issue
- Any non-functional contact path without backend is a critical issue
- Always propose fixes, not just problems
- overall_score is an integer between 0 and 100

Modular architecture checks (the site uses native ES modules):
- index.html must link the stylesheet, expose a mount point (e.g. <div id="app"></div>) and load the entry with <script type="module" src="./app.js">
- app.js must import siteData from ./data/site-data.js and import each component from ./components/*.js (relative paths WITH the .js extension)
- Each components/*.js must export ONE function that receives data and returns an HTML string (no hard-coded content, no DOM queries inside templates)
- A component with hard-coded content instead of data-driven props is a UI architecture issue
- A missing .js extension in an ES module import is a critical issue (it breaks zero-build static hosting)
- Native ES modules, import/export and CDN-only Alpine.js/HTMX are ALLOWED and must NOT be flagged as forbidden frameworks"""
...
- Preserve 100% static zero-build constraints
- Use only HTML5, CSS3 and vanilla JavaScript with native ES modules, with CDN-only Alpine.js/HTMX if necessary
- Remove React, Next.js, Vue, Angular, Svelte, Astro, Vite, Node.js, npm and package.json assumptions if present
- Preserve design system consistency
- PRESERVE the modular component architecture: index.html + app.js + data/site-data.js + components/*.js + assets/css/styles.css
- Keep the EXACT same file paths as the input (do not rename components/, data/ or assets/ files; do not flatten the structure)
- Keep ES module imports relative and WITH the `.js` extension
- Keep components data-driven (function receives data, returns HTML string) — never hard-code content back into a component
- Do not break existing structure; improve UX subtly, not radically

Output STRICT JSON with the updated files. Return ONLY the files you modified, each as a {path, content} object using the SAME paths as the input:
...
def _normalize_files(payload: dict[str, Any]) -> list[dict[str, str]]:
    ...
    if isinstance(payload.get("files"), list):
        ...
        # Direct flat format: treat every string-valued key that looks like a file path
        # (has an extension and/or a directory separator) as a generated file. This keeps
        # the modular architecture intact (app.js, data/site-data.js, components/*.js, assets/css/styles.css).
        files = []
        for key, value in payload.items():
            if not isinstance(key, str) or not isinstance(value, str):
                continue
            looks_like_path = "/" in key or "." in key.rsplit("/", 1)[-1]
            if not looks_like_path:
                continue
            # Normalize a top-level style.css → styles.css for preview server compatibility
            norm_key = "styles.css" if key == "style.css" else key
            files.append({"path": norm_key, "content": value})
    return [
        f for f in files
...
def _validate_generated_files(
    ...
    # Modular architecture: CSS may live under assets/css/ and the JS entry is app.js (ES module).
    # Fall back to the legacy flat layout (styles.css / script.js) when present.
    css_file = by_path.get("styles.css") or next(
        (f for p, f in by_path.items() if isinstance(p, str) and p.endswith(".css")), None
    )
    js_entry_file = (
        by_path.get("app.js")
        or by_path.get("script.js")
        or next((f for p, f in by_path.items() if isinstance(p, str) and p.endswith(".js")), None)
    )
    is_modular = "app.js" in by_path or any(
        isinstance(p, str) and p.startswith("components/") for p in by_path
    )
    ...
    forbidden_patterns = [
        (r"\bReact\b|react-dom|jsx", "framework React/JSX interdit"),
        (r"\bVue\b|createApp\(|vue\.global", "framework Vue interdit"),
        (r"\bAngular\b|ng-app|@angular", "framework Angular interdit"),
        (r"\bSvelte\b|svelte", "framework Svelte interdit"),
        (r"\bAstro\b|astro-island", "framework Astro interdit"),
        (r"\bNext\.js\b|__NEXT_DATA__|next/", "framework Next.js interdit"),
        (r"\bNuxt\b|__NUXT__", "framework Nuxt interdit"),
        (r"\bVite\b|/@vite|vite/client", "outil Vite interdit"),
        (r"node_modules|package\.json|npm install|yarn install|pnpm install", "dépendance Node/npm interdite"),
        (r"type=\"text/babel\"|babel", "compilation Babel interdite"),
        (r"\.tsx?\b|<script[^>]+lang=[\"']ts", "TypeScript interdit"),
    ]
    ...
    allowed_cdn_patterns = (
        ...
        "unpkg.com/@supabase/supabase-js",
        "cdn.jsdelivr.net/npm/@supabase/supabase-js",
        "www.gstatic.com/firebasejs",
    )
    external_refs = re.findall(r'''(?:src|href)=["'](https?://[^"']+)["']''', html, flags=re.IGNORECASE)
    for ref in external_refs:
        if not any(allowed in ref for allowed in allowed_cdn_patterns):
            warnings.append(f"référence externe non autorisée : {ref}")
    ...
    for path in paths:
        if path in {"package.json", "vite.config.js", "next.config.js", "astro.config.mjs"}:
            warnings.append(f"fichier build/framework interdit : {path}")
    ...
    css_path = css_file.get("path") if isinstance(css_file, dict) else None
    if css_path and css_path not in html and ".css" not in html:
        html = html.replace("</head>", f'  <link rel="stylesheet" href="./{css_path}" />\n</head>', 1)
        fixes_applied.append(f"ajout du <link> {css_path} manquant")

    js_path = js_entry_file.get("path") if isinstance(js_entry_file, dict) else None
    if js_path and js_path not in html:
        if is_modular or js_path.endswith("app.js"):
            html = html.replace("</body>", f'  <script type="module" src="./{js_path}"></script>\n</body>', 1)
            fixes_applied.append(f"ajout du <script type=module> {js_path} manquant")
        elif ".js" not in html:
            html = html.replace("</body>", f'  <script defer src="./{js_path}"></script>\n</body>', 1)
            fixes_applied.append(f"ajout du <script> {js_path} manquant")

    if is_modular and '<div id="app"' not in html and "id='app'" not in html:
        warnings.append("point de montage #app manquant dans index.html (architecture modulaire)")

    if 'data-theme' not in html and "theme-toggle" not in html and "dark mode" not in all_content.lower():
        warnings.append("dark mode toggle manquant")
    ...
    # Anchors/ids may live in index.html (flat) or inside component templates (modular),
    # so resolve them against the full generated source.
    anchor_scope = all_content if is_modular else html
    anchor_targets = set(re.findall(r'''href=["']#([^"'\s]+)["']''', anchor_scope))
    section_ids = {s.get("id") for s in sections if isinstance(s, dict) and s.get("id")}
    existing_ids = set(re.findall(r'''\bid=["']([^"'\s]+)["']''', anchor_scope)) | section_ids
