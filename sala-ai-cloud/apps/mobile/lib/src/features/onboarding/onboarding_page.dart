...
class _SectionConfig {
  ...
  Map<String, dynamic> toJson() =>
      {'type': type, 'enabled': enabled, 'data': data};
}
...
class _OnboardingPageState extends ConsumerState<OnboardingPage> {
  ...
  bool _premium = false;
  String _projectType = 'Autre activité';
  ...
  bool _sectionsCustomized = false;

  int _step = 1;
  bool _loading = false;
  String? _createdProjectId;
  String? _assignedDomain;
  ...
  static const _stacks = [
    _StackOption(
      ...
      label: 'Site simple',
      desc:
          'Une seule page claire avec toutes les informations essentielles. Idéal pour présenter rapidement ton activité.',
    ...
    _StackOption(
      ...
      label: 'Site complet',
      desc:
          'Plusieurs pages pour détailler tes services, ton histoire, tes réalisations et tes contacts.',
  ...
  List<_SectionConfig> _buildSectionCatalogue(String sector) {
    final all = [
      ...
      _SectionConfig(
          ...
          enabled: true,
          data: {'title': 'Nos services', 'items': <Map<String, String>>[]}),
      _SectionConfig(
          type: 'about',
          ...
          enabled: false,
          data: {'title': 'À propos', 'body': '', 'founder_name': ''}),
      _SectionConfig(
          type: 'testimonials',
      ...
      _SectionConfig(
          ...
          enabled: false,
          data: {'title': 'Nos réalisations', 'style': 'grid'}),
      _SectionConfig(
          type: 'pricing',
          label: 'Tarifs / Plans',
          ...
          enabled: false,
          data: {'title': 'Nos tarifs', 'plans': <Map<String, dynamic>>[]}),
      _SectionConfig(
          type: 'faq',
          label: 'FAQ',
      ...
      _SectionConfig(
          ...
          enabled: true,
          data: {'title': 'Contactez-nous', 'subtitle': 'Réponse sous 24h'}),
      _SectionConfig(
          type: 'booking',
          ...
          color: const Color(0xFF0EA5E9),
          enabled: false,
          data: {'title': 'Réserver', 'booking_url': ''}),
      _SectionConfig(
          type: 'menu',
          label: 'Carte / Menu (restaurant)',
          icon: Icons.menu_book_outlined,
          color: const Color(0xFFF97316),
          enabled: false,
          data: {'title': 'Notre carte', 'items': <Map<String, String>>[]}),
      _SectionConfig(
          type: 'team',
          ...
          color: const Color(0xFF22D3EE),
          enabled: false,
          data: {'title': 'Notre équipe', 'members': <Map<String, String>>[]}),
      _SectionConfig(
          type: 'stats',
      ...
      _SectionConfig(
          ...
          enabled: false,
          data: {'title': '', 'cta': 'Commencer maintenant'}),
    ];
    ...
    if (lower.contains('restaurant') ||
        ...
        lower.contains('bar')) {
      for (final s in all) {
        if (['menu', 'gallery', 'booking', 'testimonials'].contains(s.type))
          s.enabled = true;
      }
    } else if (lower.contains('médical') ||
        lower.contains('kiné') ||
        lower.contains('cabinet')) {
      for (final s in all) {
        if (['team', 'booking', 'faq', 'about'].contains(s.type))
          s.enabled = true;
      }
    } else if (lower.contains('portfolio') || lower.contains('agence')) {
      for (final s in all) {
        if (['gallery', 'testimonials', 'pricing', 'about'].contains(s.type))
          s.enabled = true;
      }
    } else if (lower.contains('e-commerce') || lower.contains('boutique')) {
      for (final s in all) {
        if (['gallery', 'testimonials', 'faq', 'pricing'].contains(s.type))
          s.enabled = true;
      }
    } else if (lower.contains('fitness') || lower.contains('sport')) {
      for (final s in all) {
        if (['pricing', 'team', 'testimonials', 'gallery'].contains(s.type))
          s.enabled = true;
      }
    }
    return all;
  ...
  void initState() {
    ...
    _descriptionController.addListener(_refreshForm);
    _contactEmailController.addListener(_refreshForm);
  ...
  void dispose() {
    ...
    _descriptionController.removeListener(_refreshForm);
    _contactEmailController.removeListener(_refreshForm);
    _phoneController.removeListener(_refreshForm);
    for (final c in [
  ...
  String get _domain {
    if (_assignedDomain != null && _assignedDomain!.isNotEmpty) {
      return _assignedDomain!;
    }
    final slug = _slugify(_projectName);
    return '${slug.isEmpty ? 'mon-site' : slug}.salaai.site';
  }

  String _slugify(String value) {
    const accents = 'àáâäãåçèéêëìíîïñòóôöõùúûüýÿœæ';
    const ascii = 'aaaaaaceeeeiiiinooooouuuuyyoa';
  ...
  bool get _hasUsefulDescription =>
      _descriptionController.text.trim().length >= 20;

  bool get _contactIsValid {
    ...
    final contactEnabled = _sections
        .any((section) => section.type == 'contact' && section.enabled);
    return !contactEnabled || email.isNotEmpty || phone.isNotEmpty;
  }
  ...
  Future<void> _pickLogo() async {
    try {
      ...
      if (file.size > 2 * 1024 * 1024) {
        throw const FormatException('Le logo dépasse 2 Mo.');
      }
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(error is FormatException
                  ? error.message
                  : 'Impossible de charger ce fichier. Utilise un PNG, JPG ou SVG de moins de 2 Mo.')),
        );
      }
    }
  }
  ...
  Future<void> _generate() async {
    ...
    try {
      final existingProjectId = _createdProjectId;
      ...
      if (existingProjectId == null) {
        ...
        _assignedDomain = project['default_domain']?.toString();
      } else {
      ...
      final onboardingPayload = <String, dynamic>{
        ...
        'has_blog': _selectedPages.contains('blog') ||
            _sections
                .any((section) => section.type == 'blog' && section.enabled),
        'tone': _tone,
      ...
      await ref
          ...
          .saveOnboarding(projectId, onboardingPayload);
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Stack(
          children: [
            const Positioned(
                top: -120, left: -80, child: _Glow(color: Color(0xFF2563EB))),
            const Positioned(
                bottom: -140,
                right: -80,
                child: _Glow(color: Color(0xFF0EA5E9))),
            Center(
                ...
                child: ConstrainedBox(
                  ...
                  child: Column(
                    children: [
                      ...
                      if (ref.watch(currentJobIdProvider) != null)
                        const SizedBox(height: 20),
                      _Progress(
                          step: _step, totalSteps: 5, scale: _scale(context)),
  ...
  Widget _stepContent() {
    ...
    return switch (_step) {
      ...
      2 => _stepBrandIdentity(scale),
      3 => _stepSections(scale),
      4 => _stepContactDetails(scale),
      ...
      child: Column(
        children: [
          ...
          Container(
            ...
            child:
                Icon(Icons.auto_awesome, color: Colors.white, size: 36 * scale),
          ),
          ...
          TextField(
            ...
            decoration: _darkInput(
                'Nom du site *',
                'Ex : Boulangerie Martin, Cabinet Horizon…',
                Icons.edit_outlined),
          ),
          SizedBox(height: 6 * scale),
          Align(
            alignment: Alignment.centerLeft,
            child: Text('Adresse prévue : $_domain',
                style: TextStyle(
          ...
          Align(
            alignment: Alignment.centerLeft,
            child: Text('2. Structure du site',
                style: TextStyle(
                    color: Colors.white.withValues(alpha: 0.62),
                    fontSize: 13 * scale,
                    fontWeight: FontWeight.w700)),
          ...
          Align(
            alignment: Alignment.centerLeft,
            child: Text('3. Secteur d\'activité',
                style: TextStyle(
                    color: Colors.white.withValues(alpha: 0.62),
                    fontSize: 13 * scale,
                    fontWeight: FontWeight.w700)),
          ...
          Wrap(
            ...
            children: _sectors.map((s) {
              ...
              return GestureDetector(
                onTap: () => setState(() {
                  _projectType = s.$1;
                  final recommendations = _buildSectionCatalogue(s.$1);
                  if (_sectionsCustomized) {
                    final current = {
                      for (final item in _sections) item.type: item
                    };
                    for (final item in recommendations) {
                      final previous = current[item.type];
                      if (previous != null) {
                        item.enabled = previous.enabled;
                        item.data = Map<String, dynamic>.from(previous.data);
                      }
                    }
                  }
                  _sections = recommendations;
                }),
          ...
          SizedBox(height: 28 * scale),
          FilledButton.icon(
            onPressed: _projectNameController.text.trim().isEmpty
                ? null
                : () => setState(() => _step = 2),
      ...
      child: Column(
        ...
        children: [
          _Header(
              ...
              subtitle: 'L’essentiel pour écrire un site pertinent',
              color: const Color(0xFF0EA5E9),
              scale: scale),
          ...
          if (stacked) ...[
            TextField(
                ...
                style: TextStyle(color: Colors.white, fontSize: 16 * scale),
                decoration: _darkInput(
                    'Nom de la marque', _projectName, Icons.storefront)),
            SizedBox(height: 12 * scale),
            TextField(
                controller: _taglineController,
                style: TextStyle(color: Colors.white, fontSize: 16 * scale),
                decoration: _darkInput('Slogan', 'Votre promesse en une phrase',
                    Icons.short_text)),
          ] else
            Row(children: [
              Expanded(
                  child: TextField(
                      controller: _brandNameController,
                      style:
                          TextStyle(color: Colors.white, fontSize: 16 * scale),
                      decoration: _darkInput(
                          'Nom de la marque', _projectName, Icons.storefront))),
              ...
              Expanded(
                  child: TextField(
                      ...
                      style:
                          TextStyle(color: Colors.white, fontSize: 16 * scale),
                      decoration: _darkInput(
                          'Slogan', 'Votre promesse', Icons.short_text))),
            ]),
          ...
          TextField(
            ...
            decoration: _darkInput(
                ...
                'Ce que tu proposes, à qui et ce qui te différencie…',
                Icons.description_outlined),
          ),
          if (_descriptionController.text.isNotEmpty && !_hasUsefulDescription)
            Padding(
              padding: const EdgeInsets.only(top: 6),
              child: Text('Ajoute au moins 20 caractères pour guider l’IA.',
                  style: TextStyle(
                      color: Colors.amber.shade300, fontSize: 11 * scale)),
            ),
          SizedBox(height: 14 * scale),
          ...
          TextField(
            ...
            decoration: _darkInput(
                'Client idéal (optionnel)',
                'Ex : familles du quartier, PME, voyageurs…',
          ...
          TextField(
            ...
            style: TextStyle(color: Colors.white, fontSize: 15 * scale),
            decoration: _darkInput('Objectif principal (optionnel)',
                'Ex : obtenir des demandes de devis', Icons.flag_outlined),
          ),
          SizedBox(height: 14 * scale),

          OutlinedButton.icon(
            onPressed: () =>
                setState(() => _showAdvancedBrand = !_showAdvancedBrand),
            icon: Icon(
                _showAdvancedBrand ? Icons.expand_less : Icons.tune_outlined),
            label: Text(_showAdvancedBrand
                ? 'Masquer les options visuelles'
                : 'Logo, couleurs et police (optionnel)'),
          ),
          if (_showAdvancedBrand) ...[
            SizedBox(height: 14 * scale),
            ...
            InkWell(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: Colors.white
                      .withValues(alpha: _logoBase64 != null ? 0.08 : 0.04),
                  borderRadius: BorderRadius.circular(16),
                  border: Border.all(
                ...
                child: Column(children: [
                  if (_logoBase64 != null) ...[
                    if (_logoBase64!.startsWith('data:image/svg'))
                      const Icon(Icons.image_outlined,
                          color: AppColors.skyBlueAccent, size: 42)
                    else
                      Image.memory(base64Decode(_logoBase64!.split(',')[1]),
                          height: 48, fit: BoxFit.contain),
                    const SizedBox(height: 8),
                    Text(_logoFileName ?? 'logo',
                        style:
                    ...
                    Icon(Icons.upload_file,
                        color: Colors.white.withValues(alpha: 0.38), size: 28),
                    const SizedBox(height: 6),
                    Text('Charger un logo (PNG, SVG, JPG)',
                        style: TextStyle(
          ...
          // Style keywords
          Text('Style visuel (3 choix maximum)',
              style: TextStyle(
                  color: Colors.white.withValues(alpha: 0.62),
                  fontSize: 12,
                  fontWeight: FontWeight.w700)),
          ...
          Wrap(
            ...
            children: _styleKeywordOptions.map((kw) {
              ...
              return GestureDetector(
                onTap: () => setState(() {
                  if (sel)
                    _styleKeywords.remove(kw);
                  else if (_styleKeywords.length < 3) _styleKeywords.add(kw);
                }),
                child: AnimatedContainer(
                  duration: const Duration(milliseconds: 140),
                  padding:
      ...
      child: Column(
        ...
        children: [
          _Header(
              ...
              subtitle: '$enabledCount section(s) sélectionnée(s)',
              color: const Color(0xFF06B6D4),
              scale: scale),
          SizedBox(height: 8 * scale),
          Text(
              'Nous avons préparé une sélection adaptée à ton activité. Tu peux simplement continuer.',
              style: TextStyle(
          ...
          if (enabledCount < 3)
            Padding(
              ...
              child: Text(
                  'Sélectionne au moins 3 sections pour obtenir une page suffisamment riche.',
                  style: TextStyle(
                      color: Colors.amber.shade300, fontSize: 11 * scale)),
            ),
          SizedBox(height: 24 * scale),
          _Actions(
              ...
              next: enabledCount >= 3 ? () => setState(() => _step = 4) : null,
              nextLabel: 'Continuer'),
        ],
      ),
    );
  ...
  // ────────────────────────────────────────────────────────────────────────
  // STEP 4 — Contact & pages
  // ────────────────────────────────────────────────────────────────────────

  Widget _stepContactDetails(double scale) {
    final enoughPages = _stack != 'multipage' || _selectedPages.length >= 3;
    final canContinue = _contactIsValid && enoughPages;
    return _GlassPanel(
      ...
      child: Column(
        ...
        children: [
          _Header(
              ...
              title: 'Contact et pages',
              subtitle: 'Les dernières informations utiles',
              color: const Color(0xFF14B8A6),
              scale: scale),
          SizedBox(height: 24 * scale),
          TextField(
              ...
              style: TextStyle(color: Colors.white, fontSize: 16 * scale),
              decoration: _darkInput('Email de contact', 'contact@monsite.com',
                  Icons.mail_outline)),
          SizedBox(height: 12 * scale),
          TextField(
              ...
              style: TextStyle(color: Colors.white, fontSize: 16 * scale),
              decoration: _darkInput(
                  'Téléphone', '+33 6 00 00 00 00', Icons.phone_outlined)),
          SizedBox(height: 12 * scale),
          TextField(
              controller: _addressController,
              style: TextStyle(color: Colors.white, fontSize: 16 * scale),
              decoration: _darkInput('Adresse physique (optionnel)',
                  '12 Rue de la Paix, Paris', Icons.location_on_outlined)),
          if (!_contactIsValid)
            Padding(
              padding: const EdgeInsets.only(top: 8),
              child: Text(
                  'Ajoute un email valide ou un téléphone pour la section Contact.',
                  style: TextStyle(
                      color: Colors.amber.shade300, fontSize: 11 * scale)),
            ),
          if (_stack == 'multipage') ...[
            ...
            if (!enoughPages)
              Padding(
                padding: const EdgeInsets.only(top: 8),
                child: Text(
                    'Un site complet doit contenir au moins 3 pages, dont l’accueil.',
                    style: TextStyle(
                        color: Colors.amber.shade300, fontSize: 11 * scale)),
              ),
          ],
          ...
          OutlinedButton.icon(
            ...
            icon: Icon(
                _showSocialLinks ? Icons.expand_less : Icons.add_link_outlined),
            label: Text(_showSocialLinks
                ? 'Masquer les réseaux sociaux'
                : 'Ajouter les réseaux sociaux (optionnel)'),
          ),
          if (_showSocialLinks) ...[
            SizedBox(height: 12 * scale),
            TextField(
                ...
                style: TextStyle(color: Colors.white, fontSize: 15 * scale),
                decoration: _darkInput('Instagram', 'https://instagram.com/...',
            ...
            TextField(
                ...
                keyboardType: TextInputType.url,
                style: TextStyle(color: Colors.white, fontSize: 15 * scale),
                decoration: _darkInput('Facebook', 'https://facebook.com/...',
            ...
            TextField(
                ...
                style: TextStyle(color: Colors.white, fontSize: 15 * scale),
                decoration: _darkInput(
                    'LinkedIn', 'https://linkedin.com/in/...', Icons.language)),
          ],
          SizedBox(height: 24 * scale),
          _Actions(
              back: () => setState(() => _step = 3),
              next: canContinue ? () => setState(() => _step = 5) : null,
  ...
  Widget _stepConfirmation(double scale) {
    final enabledSections =
        _sections.where((s) => s.enabled).map((s) => s.label).toList();
    return _GlassPanel(
      ...
      child: Column(
        children: [
          Container(
            ...
            child:
                Icon(Icons.check_circle, color: Colors.white, size: 36 * scale),
          ),
          SizedBox(height: 20 * scale),
          Text('Tout est prêt !',
              style: TextStyle(
              ...
              textAlign: TextAlign.center),
          SizedBox(height: 8 * scale),
          Text('L\'IA va générer ton site selon tes spécifications exactes.',
              style: TextStyle(
                  color: Colors.white.withValues(alpha: 0.56),
                  fontSize: 14 * scale),
              textAlign: TextAlign.center),
          ...
          _SummaryRow(
              label: 'Structure',
              value: _stack == 'onepage' ? 'Une page' : 'Plusieurs pages'),
          _SummaryRow(
              label: 'Qualité', value: _premium ? 'Premium' : 'Standard'),
          ...
          _SummaryRow(
              label: 'Sections',
              value: enabledSections.isEmpty
                  ? 'Aucune'
                  : '${enabledSections.length} section(s)'),
          ...
          _ResponsiveActions(
            back: _loading ? null : () => setState(() => _step = 4),
            next: _loading ? null : _generate,
            backLabel: 'Modifier',
            nextLabel:
                _loading ? 'Génération en cours…' : 'Lancer la génération',
            nextIcon: _loading
                ? const SizedBox(
                    width: 18,
                    height: 18,
  ...
  InputDecoration _darkInput(String label, String hint, IconData icon) {
    return InputDecoration(
      ...
      focusedBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(16),
          borderSide: const BorderSide(color: AppColors.skyBlue, width: 1.6)),
    );
  }
}
      ...
      child: AnimatedContainer(
        ...
        child: ListTile(
          leading: Container(
            ...
            decoration: BoxDecoration(
              color: section.color
                  .withValues(alpha: section.enabled ? 0.22 : 0.08),
...
class _SectionConfigDialogState extends State<_SectionConfigDialog> {
  ...
  void initState() {
    ...
    for (final key in [
      ...
      if (_data.containsKey(key)) {
        _controllers[key] =
            TextEditingController(text: _data[key] as String? ?? '');
      }
    }
  ...
  void _save() {
    ...
    // Put list back
    final listKey = _data.containsKey('plans')
        ? 'plans'
        : _data.containsKey('members')
            ? 'members'
            : 'items';
            ...
            children: [
              ...
              Row(children: [
                Container(
                  ...
                  height: 40,
                  decoration: BoxDecoration(
                      color: widget.section.color.withValues(alpha: 0.20),
                      borderRadius: BorderRadius.circular(12)),
                  child: Icon(widget.section.icon,
                      color: widget.section.color, size: 20),
                ),
                const SizedBox(width: 12),
                Expanded(
                    child: Column(
  ...
  List<Widget> _buildFields() {
    ...
    void field(String key, String label, String hint,
        {int maxLines = 1, IconData icon = Icons.edit}) {
    ...
    switch (widget.section.type) {
      case 'hero':
        field('title', 'Titre principal', 'Ex : Le goût authentique de Lyon',
            icon: Icons.title);
        field(
            'subtitle', 'Sous-titre', 'Ex : Depuis 2008, au cœur du Vieux-Lyon',
            maxLines: 2, icon: Icons.subtitles);
        field('cta_primary', 'Bouton principal', 'Ex : Réserver une table',
            icon: Icons.touch_app);
        field('cta_secondary', 'Bouton secondaire', 'Ex : Voir le menu',
            icon: Icons.touch_app_outlined);
        break;

      case 'services':
        field('title', 'Titre de la section', 'Ex : Nos services',
            icon: Icons.title);
      ...
      case 'about':
        ...
        field('body', 'Texte de présentation',
            'Qui vous êtes, votre histoire, vos valeurs...',
            maxLines: 5, icon: Icons.text_fields);
        field(
            'founder_name', 'Nom du fondateur (optionnel)', 'Ex : Paul Martin',
      ...
      case 'pricing':
        ...
        widgets.add(_ListEditor(
          ...
          fields: const [
            ...
            ('features', 'Fonctionnalités (séparées par ;)')
          ],
      ...
      case 'contact':
        ...
        field('subtitle', 'Sous-titre', 'Ex : Réponse sous 24h',
            icon: Icons.subtitles);
        break;

      case 'booking':
        field('title', 'Titre', 'Ex : Réserver une table', icon: Icons.title);
        field('subtitle', 'Sous-titre', 'Ex : Disponibilités en ligne 24h/24',
            icon: Icons.subtitles);
        field('booking_url', 'Lien de réservation externe',
            'https://calendly.com/...',
            icon: Icons.link);
        break;
      ...
      case 'stats':
        widgets.add(_ListEditor(
          ...
          fields: const [
            ('value', 'Valeur (ex: 500+)'),
            ('label', 'Libellé (ex: Clients satisfaits)')
          ],
      ...
      case 'cta_banner':
        field('title', 'Titre de la bannière', 'Ex : Prêt à démarrer ?',
            icon: Icons.title);
        field('cta', 'Texte du bouton', 'Ex : Commencer maintenant',
            icon: Icons.touch_app);
        break;
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            Row(children: [
              ...
              GestureDetector(
                ...
                child: Icon(Icons.delete_outline,
                    color: Colors.red.withValues(alpha: 0.65), size: 18),
              ),
            ]),
            const SizedBox(height: 8),
      ...
      if (_items.isEmpty)
        Container(
          ...
          child: Text('Aucun élément. Cliquez "+ Ajouter".',
              style: TextStyle(
...
class _Progress extends StatelessWidget {
  const _Progress(
      {required this.step, required this.totalSteps, this.scale = 1.0});
  final int step;
...
class _StackCard extends StatelessWidget {
  const _StackCard(
      {required this.stack, required this.selected, required this.onTap});
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: selected
              ? stack.color.withValues(alpha: 0.20)
              : Colors.white.withValues(alpha: 0.06),
          borderRadius: BorderRadius.circular(20),
          border: Border.all(
              color:
                  selected ? stack.color : Colors.white.withValues(alpha: 0.12),
              width: selected ? 2 : 1),
        ),
        child: Stack(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(
              ...
              height: 40,
              decoration: BoxDecoration(
                  color: stack.color, borderRadius: BorderRadius.circular(12)),
        ...
        child: Row(children: [
          ...
          Expanded(
              child: Column(
                ...
                Row(children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                        color: color.withValues(alpha: 0.18),
                        borderRadius: BorderRadius.circular(999)),
                    child: const Text('Haute qualité',
                        style: TextStyle(
                            color: color,
                            fontSize: 10,
                            fontWeight: FontWeight.w800)),
                ...
                const SizedBox(height: 4),
                Text(
                    'Ajoute stratégie, UX avancée, contrôle qualité et raffinement à la structure choisie.',
                    style: TextStyle(
...
class _Actions extends StatelessWidget {
  const _Actions(
      {required this.back, required this.next, required this.nextLabel});
  ...
  @override
  Widget build(BuildContext context) {
    return _ResponsiveActions(
        back: back, next: next, backLabel: 'Retour', nextLabel: nextLabel);
  }
}
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      if (stacked) {
        return Column(
            crossAxisAlignment: CrossAxisAlignment.stretch,
            children: [nextBtn, const SizedBox(height: 10), backBtn]);
      }
      return Row(children: [
        Flexible(child: backBtn),
        const SizedBox(width: 12),
      ...
      DropdownButtonFormField<String>(
        ...
        decoration: InputDecoration(
          ...
          focusedBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(16),
              borderSide:
                  const BorderSide(color: AppColors.skyBlue, width: 1.6)),
        ),
        items: options
            .map((f) => DropdownMenuItem<String>(
                value: f.label, child: Text('${f.label} · ${f.category}')))
            .toList(),
        onChanged: (val) {
...
class _ColorPickersState extends State<_ColorPickers> {
  void _openColorPicker(
      String title, Color initial, ValueChanged<Color> onChanged) {
    Color picked = initial;
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        backgroundColor: const Color(0xFF0F172A),
        title: Text('Choisir la couleur $title',
            style: const TextStyle(color: Colors.white)),
      ...
      child: Row(children: [
        ...
        Flexible(
            child: Text(value,
                ...
                style: TextStyle(
                    color: accent ? AppColors.skyBlueAccent : Colors.white,
                    fontWeight: FontWeight.w700))),
      ]),
    );
  }
